    ...
    if( function_exists('um_profile_id') && $args->theme_location == 'main-navigation' ){
		...
		<li class="menu-item-has-children">
			<?php if ( um_profile_id() ) : ?>
				<a href="<?php echo esc_url(home_url('user/')); ?>"><i class='icon-User'></i><?php echo um_get_display_name( um_profile_id() ); ?></a>
			<?php else: ?>
				<a href="<?php echo esc_url(home_url('login/')); ?>"><i class='icon-User'></i></a>
			<?php endif; ?>
			<ul class="sub-menu">
				<?php if ( um_profile_id() ) : ?>
				<li><a href="<?php echo esc_url(home_url('user/')); ?>"><?php esc_html_e('Profile', 'couponhut') ?></a></li>
				<?php 
				$template_url = fw_ssd_get_submit_template_url();
				if ( $template_url ) : ?>
				<li><a href="<?php echo esc_url(home_url('submit/')); ?>"><?php esc_html_e('Submit', 'couponhut') ?></a></li>
				<?php endif; ?>
				<li><a href="<?php echo esc_url(home_url('logout/')); ?>"><?php esc_html_e('Log Out', 'couponhut') ?></a></li>
				<?php else : ?>
				<li><a href="<?php echo esc_url(home_url('login/')); ?>"><?php esc_html_e('Log In', 'couponhut') ?></a></li>
				<li><a href="<?php echo esc_url(home_url('register/')); ?>"><?php esc_html_e('Register', 'couponhut') ?></a></li>
				<?php endif; ?>
			</ul>
		</li>
    	<?php
