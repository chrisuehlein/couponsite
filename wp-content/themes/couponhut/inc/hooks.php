...
if ( ! function_exists( '_action_ssd_theme_setup' ) ) {
	function _action_ssd_theme_setup() {
        ...
        add_image_size( 'ssd_single-post-image', 700, 300, true );
...
function _filter_fw_ssd_acf_json_save_point( $path ) {
...
add_filter('acf/settings/save_json', '_filter_fw_ssd_acf_json_save_point');

/**
*  ACF Load JSON
*/

function _filter_fw_ssd_acf_json_load_point( $paths ) {
...
add_filter('acf/settings/load_json', '_filter_fw_ssd_acf_json_load_point');
...
function _filter_ssd_acf_update_value( $value, $post_id, $field  )
{
	return wp_kses_post($value);
}

// add_filter('acf/update_value/type=wysiwyg', '_filter_ssd_acf_update_value', 10, 3);
